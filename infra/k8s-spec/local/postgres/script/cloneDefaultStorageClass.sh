...
kubectl apply -f  ../pv/clone-storageClass-script.yaml 
...
kubectl exec clone-sc -- /scripts/duplicate-default-storage-class.sh > customStorageClass.json

# apply the new sc:
kubectl apply -f customStorageClass.json

# and remove the cloning pod:
kubectl delete -f  ../pv/clone-storageClass-script.yaml